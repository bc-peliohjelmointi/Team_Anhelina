...
public class MainDoorLever : MonoBehaviour
{
    ...
    private bool isDoorOpen = false, isMoving = false;
    ...
    {
        if (leverHandle != null)
            leverHandle.localRotation = Quaternion.Lerp(leverHandle.localRotation, leverTargetRotation, Time.deltaTime * leverSpeed);
    }
    ...
    public void PullLever()
    {
        ...
        {
            if (!isDoorOpen) { leverTargetRotation = Quaternion.Euler(upRotation); if (leverSound != null) audioSource.PlayOneShot(leverSound, soundVolume); StartCoroutine(OpenDoor()); }
        }
        else { if (errorSound != null) audioSource.PlayOneShot(errorSound, soundVolume); }
    }
    ...
    IEnumerator OpenDoor()
    {
        ...
        float elapsed = 0f, duration = 1f / doorSpeed;
        while (elapsed < duration)
        {
            elapsed += Time.deltaTime;
            door.localPosition = Vector3.Lerp(startPos, openPosition, doorCurve.Evaluate(Mathf.Clamp01(elapsed / duration)));
            yield return null;
        }
        door.localPosition = openPosition;
        isDoorOpen = true; isMoving = false;
