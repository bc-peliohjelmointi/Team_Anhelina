...
public class AlarmSystem : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < alarmRenderers.Length; i++)
            if (alarmRenderers[i] != null) rendererMaterials[i] = alarmRenderers[i].material;
        SetLights(false);
    }
    ...
    IEnumerator FlashLights()
    {
        while (true)
        {
            SetLights(true); yield return new WaitForSeconds(1f / flashSpeed);
            SetLights(false); yield return new WaitForSeconds(1f / flashSpeed);
        }
    }

    void SetLights(bool on)
    {
        foreach (Light l in alarmLights) { if (l == null) continue; l.enabled = on; l.color = alarmColor; }
        for (int i = 0; i < alarmRenderers.Length; i++)
        {
            if (alarmRenderers[i] == null || rendererMaterials[i] == null) continue;
            if (on) { rendererMaterials[i].EnableKeyword("_EMISSION"); rendererMaterials[i].SetColor(emissionProperty, alarmColor * 3f); }
            else { rendererMaterials[i].DisableKeyword("_EMISSION"); rendererMaterials[i].SetColor(emissionProperty, Color.black); }
        }
    }
}